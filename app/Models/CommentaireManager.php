...
//affiche tout  de la table commentaires ou l'id_jeu =  $id rangé par ordre decroissant dans une limite de 4 commentaires
    public function getCommentaires($id){
        $bdd = $this->bdConnect();
        $req = $bdd->prepare('SELECT  `id`, `content`, `id_jeu`, `notepost`, `pseudo`, `totalContent`FROM commentaires WHERE id_jeu=? ORDER BY id DESC limit 4');
        $req->execute(array($id));
        return $req;
    }
...
//affiche tout  de la table commentaires et la categorie(en tant que categorieJeu) de la table jeux ou id=id_jeu le tout ou l'id_jeu =  $id rangé par ordre decroissant 
    public function getAllcommentaires($id_jeu){
        $bdd = $this->bdConnect();
        $req = $bdd->prepare("SELECT  `id`, `content`, `id_jeu`, `notepost`, `pseudo`, `totalContent`,(SELECT `categorie` FROM `jeux` WHERE `id`=`id_jeu`)AS categorieJeu FROM commentaires WHERE id_jeu=? ORDER BY id DESC ");
...
//pour recuperer la moyenne des notes(arrondie à la première décimale) des commentaires sur le jeu avec l'id=$id
